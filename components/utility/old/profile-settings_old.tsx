import { AdminPreferences } from "../../managements/admin-preferences"
...
export const ProfileSettings: FC<ProfileSettingsProps> = ({}) => {
  ...
  const handleSave = async () => {
    ...
    // if (!apiKeys) {
    //   const updatedApiKeys = await createApiKeys(apiKeyData)
    // } else {
    //   const updatedApiKeys = await updateApiKeys(apiKeys.id, apiKeyData)
    // }
      ...
      >
        ...
        <div className="mt-6 flex items-center">
          <div className="flex items-center space-x-1">
            ...
            {/* If login user has admin-authority */}
            {userInfo?.is_admin ? (
              <WithTooltip
                display={<div>Managements</div>}
                trigger={<AdminPreferences />}
              />
            ) : null}
