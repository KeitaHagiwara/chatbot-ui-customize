...
export async function getServerProfile() {
  ...
  const { data: apikeys } = await supabase.from("apikeys").select("*").single()

  if (!apikeys) {
    throw new Error("API Keys not registered")
  }

  const profileWithKeys = addApiKeysToProfile(apikeys)

  return profileWithKeys
}

function addApiKeysToProfile(apikeys: Tables<"apikeys">) {
  const apiKeys = {
    [VALID_ENV_KEYS.OPENAI_API_KEY]: "openai_api_key",
    [VALID_ENV_KEYS.ANTHROPIC_API_KEY]: "anthropic_api_key",
    [VALID_ENV_KEYS.GOOGLE_GEMINI_API_KEY]: "google_gemini_api_key",
  ...
  for (const [envKey, profileKey] of Object.entries(apiKeys)) {
    if (process.env[envKey]) {
      ;(apikeys as any)[profileKey] = process.env[envKey]
    }
  }
